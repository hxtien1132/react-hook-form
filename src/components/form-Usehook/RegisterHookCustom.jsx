...
const RegisterHook = () => {
  ...
  const onSubmitHandler = (values) => {
    if (!isValid) return ;
    return new Promise((resolve) => {  //useForm sử dụng promise
      setTimeout(() => {
    console.log(values);
        resolve();
        reset({
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-3 mb-5">
        ...
        <InputHook
          name="username"  
          control={control} // bắt buộc truyền 2 trường name:"" và control={control}
      ...
      <button
        className={`w-full p-5 mt-5 font-semibold text-white bg-blue-500 rounded-lg ${
          isSubmitting ? "opacity-50" : "" // đang submit mờ
        }`}
        disabled={isSubmitting} //khi submitting k dk nhấn nx
      >
        {isSubmitting ? (
          <div className="w-5 h-5 mx-auto  border-solid border-2 border-t-2 border-white rounded-full border-t-transparent animate-spin"></div>
        ) : (
          "Submit"
        )}
      </button>
